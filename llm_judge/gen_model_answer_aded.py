...
def aded_forward(input_ids, model, tokenizer, max_new_token, temperature, top_p, datastore, num_draft, token_spans, max_steps=1024):
...
#    model = dispatch_model(model,device_map=device_map)
    ...
    for _ in tqdm(range(3)):
        ...
        for j in range(len(question["turns"])):
            ...
            try:
                output_ids, new_token, idx, _, start_time = aded_forward(
    ...
    for question in tqdm(questions):
        ...
        for i in range(num_choices):
            ...
            for j in range(len(question["turns"])):
                ...
                try:
                    ...
                    output_ids, new_token, idx, accept_length_tree, start_time = aded_forward(
                        torch.as_tensor(input_ids).cuda(),
                        model,
                        tokenizer,
                        max_new_token,
        ...
        print(os.path.expanduser(answer_file))
...
if __name__ == "__main__":
    ...
    args.model_id = "aded-" + args.model_id+"-temperature-"+str(args.temperature)+"-top_p-"+str(args.top_p) + '-' +str(args.id)
